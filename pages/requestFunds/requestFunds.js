...
Page({
  ...
  data: {
    ...
    array: [],
    tableData: [
      { 'id': '1', 'name': '报关费', 'type': '￥', 'number': '80' },
      { 'id': '2', 'name': '拖车费', 'type': '￥', 'number': '120' },
      { 'id': '3', 'name': '合计', 'type': '￥', 'number': '200' }
    ],
    //驳回弹窗
    showModal: false,
    showModalText: "驳回该请款审核",
    Remark: '',
    //下拉加载
  ...
  filterConfirm: function () {
    ...
    if (this.data.isSelfDefineTime==false){
      ...
      if(radioBeginDate==''){
        ...
        v ='Request,Fee_CQK05,'+ radioBeginDate + "," + endDate;
      }
    }else{
      pre = 'DUNNINGORREQUESTS==@0 and NODECODE==@1 and BILLDATE >= MDFunctions.ParseDateTime(@2) and MDFunctions.ParseDateTime(@3) >= BILLDATE ';
      v ='Request,Fee_CQK05,'+ beginDate + "," + endDate;
    }
